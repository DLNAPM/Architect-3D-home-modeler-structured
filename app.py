#!/usr/bin/env python3
"""
Architect 3D Home Modeler – Powered by Google AI (Photorealism Upgrade)
- This version uses aggressive prompt engineering to force photorealism.
- Commands eye-level perspective and sharp focus to prevent satellite/blurry views.
- Implements a strong negative prompt in the API call.
"""
...
def build_prompt(subcategory: str, options_map: dict, description: str, plan_uploaded: bool):
    """Builds a highly detailed and context-aware prompt to force photorealism."""
    
    # --- PROMPT UPGRADE --- Using more forceful and technical language
    realism_command = "An ultra-realistic, professional architectural photograph of a residential home. The image must be tack-sharp focus with a deep depth of field; every detail from foreground to background is crisp and in focus. Emulate a shot taken on a Sony A7R IV with a sharp 35mm G Master prime lens. The lighting is soft, natural, golden hour light, casting long, gentle shadows. The image must have a cinematic quality, with photorealistic textures (wood grain, concrete texture, glass reflections) suitable for a feature in Architectural Digest."
    selections = ", ".join([f"{k} is {v}" for k, v in options_map.items() if v and v not in ["None", ""]])
    
    view_context = ""
    negative_prompt_parts = ["cartoon", "illustration", "3d render", "unrealistic", "blurry", "distorted", "watermark", "text", "blur", "out of focus", "smudged"]

    if subcategory == "Front Exterior":
        view_context = f"This is an eye-level, street-level perspective of the {subcategory}, clearly showing the driveway, garage, and front entrance."
        ...
        view_context = f"This is an eye-level perspective of the {subcategory} from the backyard, with a focus on outdoor living areas like the patio or pool area."
        negative_prompt_parts.extend(["front yard", "driveway", "street view"])
    else:
...
def generate_image_via_google_ai(prompt: str, negative_prompt: str) -> str:
    ...
    # --- API UPGRADE --- Adding the negative_prompt parameter
...
def generate():
    ...
    for subcat in ["Front Exterior", "Back Exterior"]:
        try:
            prompt, negative_prompt = build_prompt(subcat, {}, description, False)
